...
in
	...
	then (print "1- preprocess converts a LineSegment to a Point successfully\n")
	else (print "2- preprocess does not convert a LineSegment to a Point succesfully\n")
end;
...
in
	...
	then (print "1- preprocess does not flip an proper LineSegment successfully\n")
	else (print "2- preprocess flips an proper LineSegment successfully\n")
end;

let
	val LineSegment(a,b,c,d) = preprocess_prog (LineSegment(3.2,4.1,3.3,~4.1))
	val LineSegment(e,f,g,h) = LineSegment(3.2,4.1,3.3,~4.1)
in
	if real_equal(a,e) andalso real_equal(b,f) andalso real_equal(c,g) andalso real_equal(d,h)
	then (print "1- preprocess does not flip an proper LineSegment successfully\n")
	else (print "2- preprocess flips an proper LineSegment successfully\n")
...
in
	if real_equal(a,c) andalso real_equal(b,d)
	then (print "1- eval_prog with empty environment worked\n")
	else (print "2- eval_prog with empty environment is not working properly\n")
end;
...
in
	if real_equal(a,c) andalso real_equal(b,d)
	then (print "1- eval_prog with 'a' in environment is working properly\n")
	else (print "2- eval_prog with 'a' in environment is not working properly\n")
end;
...
in
	if real_equal(a,c) andalso real_equal(b,d)
	then (print "1- eval_prog with shadowing 'a' in environment is working properly\n")
	else (print "2- eval_prog with shadowing 'a' in environment is not working properly\n")
end;
